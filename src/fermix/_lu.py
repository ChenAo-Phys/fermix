...
returns what _inverse needs to rebuild the packed LU."""
...
def _lu_inner_kernel(
    ...
    *,
    ...
    def step(j, carry):
        ...
        rowp_parts = [
            jnp.sum(jnp.where(tr[:, None] == p, W, 0.0), axis=0)
            for tr, W in zip(trs, Ws)
        ]
        rowp = functools.reduce(jnp.add, rowp_parts)
        ...
        inv = lax.select(pivot == 0.0, f32(0.0), 1.0 / pivot)
        Ws = [
            jnp.where((ci2 == j) & un[:, None], (col * inv)[:, None], W)
            for W, col, un in zip(Ws, cols, unas)
        ]
        return Ws, Ut, piv_i, poss, unas, sign, logabs

    Ut0 = jnp.zeros((bi, bi), f32)
    ...
    for c, (off, h) in enumerate(chunks):
        if off < 0:
            ...
            # block's U rows)
            alive = (trs[c] >= 0)[:, None]
            plgpu.store(p_ref.at[rows(off, h), pl.ds(c0, bi)], Ws[c], mask=alive)
        else:
...
def _lu_update_kernel(
    p_ref, q_ref, piv_ref, po_ref, *, n, r0, b, bi, ch, prec, lo, mid, hi
):
    ...
    if K == 16:
        ...
        Wl = plgpu.load(p_ref.at[pl.ds(rc, ch), pl.ds(c_lo, 16)], mask=rmask, other=0.0)
        upd = _dot(Wl, U, prec)
        ...
        Rb = p_ref[pivB, pl.ds(c_mid, Wd)] - _dot(L_ba, Ua, prec)
        Ub = _dot(Linv_b, Rb, prec)
        rows_a = p_ref.at[pl.ds(rc, ch), pl.ds(c_lo, 16)]
        rows_b = p_ref.at[pl.ds(rc, ch), pl.ds(c_lo + 16, 16)]
        Wa = plgpu.load(rows_a, mask=rmask, other=0.0)
        Wb = plgpu.load(rows_b, mask=rmask, other=0.0)
        upd = _dot(Wa, Ua, prec) + _dot(Wb, Ub, prec)
    T = plgpu.load(p_ref.at[pl.ds(rc, ch), pl.ds(c_mid, Wd)], mask=rmask, other=0.0)
    # never write the pivot rows: other programs gather A from them (cross-program RAW
    # hazard otherwise)
    hits = jnp.sum((rows[:, None] == piv_src[None, :]).astype(jnp.int32), axis=1)
    keep = (hits == 0) & rvalid
    dst = p_ref.at[pl.ds(rc, ch), pl.ds(c_mid, Wd)]
    plgpu.store(dst, T - upd, mask=keep[:, None])
...
def _lu_urow_kernel(p_ref, piv_ref, q_ref, out_ref, *, n, r0, b, tn, prec):
    ...
    for i in range(nblk):
        ...
        R = plgpu.load(p_ref.at[piv_i, pl.ds(c0, tn)], mask=cmask, other=0.0)
        for s_ in range(i):
        ...
        Ui = _dot(Linv, R, prec)
        U_list.append(Ui)
        dst = q_ref.at[pl.ds(r0 + i * bu, bu), pl.ds(c0, tn)]
        plgpu.store(dst, Ui, mask=cmask)
...
def _lu_gemm_kernel(p_ref, q_ref, src_ref, out_ref, *, n, r0, b, tm, tn, prec):
    ...
    C = plgpu.load(p_ref.at[src, cols], mask=mask, other=0.0)
    C = C - _dot(Lt, U, prec)
    plgpu.store(q_ref.at[pl.ds(s0 + i0, tm), cols], C, mask=mask)
...
def _inner_call(
    ...
    *,
    ...
    out_shape = [mat, mat, idx_n, sl_shape, idx_b, idx_n]
...
def _fresh_wrapper(
    kern, p_ref, po_ref, qo_ref, poso_ref, slo_ref, pivo_ref, src_ref, *, B
):
...
def _lu_core(A, n, prec, unroll_steps, block=None, factors=False) -> tuple:
    """Blocked LU of the (B, n, n) batch; returns (sign, logabs) and, with factors=True,
    also the raw material for the packed LU (see _packed_lu): the two ping-pong buffers,
    per-block pivot rows / compaction maps and a snapshot of each block's panel columns
    taken before its factorisation (the pivot rows' U part is recomputed from it)."""
    ...
    N = P.shape[1]
    base_kw = dict(b=b, bi=bi, unroll_steps=unroll_steps)  # constant across blocks
    ...
    for k in range(nb):
        ...
        upd_kw = dict(r0=r0, b=b, bi=bi, ch=64, prec=prec, num_warps=4)
        if factors:
