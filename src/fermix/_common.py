"""Shared constants and helpers for the fermix kernels (row-tile layout cost model, dot
precision, small register-tile triangular inverses, block specs, input preparation and
padding)."""
...
BLOCK = 32
INNER = 8
# extra rows equivalent of one more register row-chunk in the LU panel step
LU_CHUNK_COST = 64
# same for the pf pair step (more reductions per step; small tail chunks on 8 warps are
# pure overhead)
PF_CHUNK_COST = 512
...
def _argmax_chunks(cands, offs):
    ...
    best_p = jnp.argmax(cands[0], axis=0).astype(jnp.int32) + offs[0]
...
def _embed(A, n, pad_block, b=BLOCK):
    """Pad A (B,n,n) to N = ceil_b(n) with `pad_block` (identity / skew identity) in the
    trailing rows/cols."""
    N = -(-n // b) * b
    if N == n:
        return A, N // b
...
def _panel_warps(m):
    """1 warp per matrix up to 256 active rows (warp-shuffle reductions); wider panels
    need more warps to avoid register spills (<=512 rows -> 4, above -> 8)."""
    return 1 if m <= 256 else (4 if m <= 512 else 8)


def _lu_block(n):
    """Outer block size: 32 up to n=256 (fewer inter-panel launches), 64 above (halves
    trailing-update traffic)."""
...
def _skew_pad(m):
    ...
    neutral padding)."""
    eye = jnp.eye(m // 2, dtype=f32)
    pair = jnp.array([[0.0, 1.0], [-1.0, 0.0]], f32)
