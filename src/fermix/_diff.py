...
derivatives that stay finite for singular inputs). ``fast`` selects the Pallas kernels (CUDA, float32); otherwise
the generic XLA implementations in _fallback are used with the same derivative rules."""
import functools
import jax
...
def _on_cuda(x, kernels, generic):
    """Run ``kernels`` when lowering for CUDA and ``generic`` elsewhere (a committed CPU array reaching a traced call
    would otherwise hit the Pallas lowering); both must return identical shapes/dtypes."""
...
def _slogdet_core(A, prec, unroll_steps, block, fast):
    ...
    if fast:
        return _on_cuda(A, lambda A: _lu_core(A, n, prec, unroll_steps, block), _slogdet_generic)
...
def _slogpf_core(S, prec, upd_warps, fast):
    ...
    if fast:
        return _on_cuda(S, lambda S: _pf_core(S, n, prec, upd_warps=upd_warps), _slogpf_generic)
...
def _parts(A, n, prec, unroll_steps, block, fast, zero_singular=True):
    if fast and _inverse.GRAD_INVERSE == "lu":
        ...
        return _on_cuda(A, lambda A: _lu_parts(A, n, prec, unroll_steps, block, zero_singular),
                        lambda A: _lu_parts_generic(A, N, zero_singular))
...
def _slogpf_diff(n, prec, upd_warps, fast):
    ...
    def f_jvp(primals, tangents):
        ...
        if n <= PF_POLY_MAX:
            ...
            sign, logabs = f(S)
            _, _, invT, _, _, _ = _parts(S, n, prec, True, None, fast)      # LU of the skew matrix (pivoting handles it)
...
def _det_gradT(sign, logabs, invT, LU, g0, zero, n):
    ...
    def singular(_):
        ...
        E = jnp.stack([jax.nn.one_hot(k1, N, dtype=dt), jax.nn.one_hot(k2, N, dtype=dt)], axis=2)
...
def _pf_gradT(sign, logabs, invT, LU, g0, zero, S, n, pf_core):
    ...
    rows/cols i, j), evaluated with a forward slogpf (``pf_core``) on that minor. Three or more zero pivots
    (rank <= n-4) give 0."""
    B, N, _ = LU.shape
    dt = LU.dtype
    ...
    def singular(_):
        ...
        E = jnp.stack([jax.nn.one_hot(k1, N, dtype=dt), jax.nn.one_hot(k2, N, dtype=dt)], axis=2)       # (B, N, 2)
        mask = E[:, :, 0] + E[:, :, 1]
...
def _det_diff(n, prec, unroll_steps, block, fast):
    ...
    def f_jvp(primals, tangents):
        ...
        if n <= DET_POLY_MAX:
            ...
            sign, logabs, invT, LU, g0, zero = _parts(A, n, prec, unroll_steps, block, fast, zero_singular=False)
...
def _pf_diff(n, prec, upd_warps, fast):
    ...
    def f_jvp(primals, tangents):
        ...
        if n <= PF_POLY_MAX:
            ...
            _, _, invT, LU, g0, zero = _parts(S, n, prec, True, None, fast, zero_singular=False)
            G = _pf_gradT(sign, logabs, invT, LU, g0, zero, S, n, lambda Sm: _slogpf_core(Sm, prec, upd_warps, fast))
