"""Generic XLA implementations (any backend, float32 or float64) used when the Pallas
kernels do not apply: LU-based slogdet, a masked batched Parlett-Reid slogpf, and the LU
parts (packed factors, permutation, zero-pivot mask, U~-based inverse) consumed by the
singular-safe gradients in _diff."""
...
def _lu_parts_generic(A, N, zero_singular=True):
    ...
    _inverse._lu_parts, from lax.linalg.lu on A padded with the identity to N x N
    (N >= n, matching the kernels' block padding): A[g0] = L U, invT = A^-T (leading
...
def _slogpf_generic(S):
    ...
    init = (S, jnp.ones(B, dt), jnp.zeros(B, dt))
