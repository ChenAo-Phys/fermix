"""Public API: slogdet, slogpf, det, pf. Batched, differentiable; Pallas kernels on CUDA GPUs for float32, a generic
XLA implementation (with a warning) elsewhere."""
import functools
import warnings
import jax
...
class FermixFallbackWarning(UserWarning):
    """The fast Pallas kernels do not apply (dtype or device); the generic XLA implementation is used."""


def _prep(a):
    a = jnp.asarray(a)
    if a.dtype not in (jnp.float32, jnp.float64):
        raise TypeError(f"fermix supports float32 (kernels) and float64 (generic path), got {a.dtype}")
...
def _use_kernels(a):
    """True when the Pallas kernels apply (float32 on a CUDA GPU); otherwise warn and return False. For traced inputs
    (inside jit/grad) only the default device is visible; a committed CPU array reaching such a call still runs the
    generic path (the kernel branch is selected at lowering time), just without the warning -- use
    ...
    if isinstance(a, jax.core.Tracer) or not isinstance(a, jax.Array):
        dev = jax.config.jax_default_device                        # traced input: only the default device is known
        platforms = {dev.platform if dev is not None else jax.default_backend()}
    else:
        platforms = {d.platform for d in a.devices()}
    if platforms != {"gpu"}:
        reasons.append(f"device platform {'/'.join(sorted(platforms))} (the kernels need a CUDA GPU)")
    if reasons:
        warnings.warn("fermix: " + "; ".join(reasons) + " -- falling back to the generic XLA implementation "
                      "(LU / Parlett-Reid in jax.numpy; same results and singular-safe gradients, but much slower).",
                      FermixFallbackWarning, stacklevel=3)
        return False
    return True
...
@functools.partial(jax.jit, static_argnames=("prec", "unroll_steps", "block", "fast"))
...
@functools.partial(jax.jit, static_argnames=("prec", "skew_symmetrize", "upd_warps", "fast"))
...
def slogdet(a, *, prec="tf32x3", unroll_steps=True, block=None):
    """Sign and log|det| of a batch of matrices ``a`` of shape (..., n, n). Returns (sign, logabsdet).

    float32 on a CUDA GPU runs the Pallas kernels; other dtypes (float64) or devices fall back to a generic
    jax.numpy LU with a :class:`FermixFallbackWarning`. prec: "tf32x3" (default; tensor cores, fp32-level accuracy)
    or "ieee" (exact fp32 FMA) for the kernels' block GEMM updates. block: outer LU block size 32/64 (default 32 for
...
def slogpf(a, *, prec="tf32x3", skew_symmetrize=True, upd_warps=4):
    """Sign and log|pf| of a batch of skew-symmetric matrices (even n; odd n gives (0, -inf)).

    The input is skew-symmetrised as (a - a^T)/2 unless ``skew_symmetrize=False`` (then it must be exactly
    skew-symmetric). n <= 6 uses the explicit polynomial. Kernels for float32 on CUDA, generic Parlett-Reid
    (with a :class:`FermixFallbackWarning`) otherwise.
...
def det(a, *, prec="tf32x3", unroll_steps=True, block=None):
    """Determinant of a batch of matrices (same options and fallback as :func:`slogdet`). Overflows float32 for
    large, badly scaled matrices -- prefer slogdet then.
...
def pf(a, *, prec="tf32x3", skew_symmetrize=True, upd_warps=4):
    """Pfaffian of a batch of skew-symmetric matrices (same options and fallback as :func:`slogpf`; odd n gives 0).
