"""Forward and gradient timings of the fermix kernels against jnp.linalg.slogdet and,
for the Pfaffian (which jax.numpy has no equivalent of), fermix's own generic XLA
Parlett-Reid path -- so the benchmark needs no package outside this repo. Usage:
...
from fermix._fallback import _slogpf_generic
...
if __name__ == "__main__":
    ...
    fwd_head = f"{'slogdet':>8} {'jnp':>8} | {'slogpf':>8} {'generic':>8}"
    ...
    for n in (64, 128, 192, 256):
        ...
        t_gen = bench(jax.jit(_slogpf_generic), S, n_rep=2)
        t_gdet = bench(g_det, A)
        t_gjnp = bench(g_jnp, A, n_rep=3)
        t_gpf = bench(g_pf, S)
        fwd = f"{t_det:>8.2f} {t_jnp:>8.2f} | {t_pf:>8.2f} {t_gen:>8.1f}"
