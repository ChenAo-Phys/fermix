...
- The code should be efficient under `jax.vmap`
- Backward priority: stability > forward-backward match > efficiency
...
- **Every path must stay cheap under `jax.vmap`.** Frameworks vmap a per-sample function, so inside the vmap each
  call has B = 1 and JAX rebuilds the batch: a `pallas_call` gets a leading grid axis (neutral), but a `lax.cond` on a
  batched predicate becomes a `select` that evaluates *both* branches on every call. Batch-level branch predicates
  therefore go through `_common.batch_any` (a `custom_vmap` whose rule also reduces over the mapped axis, so the cond
  stays a cond and the branch is chosen for the vmapped batch as a whole, exactly as in a batched call); the branches
  must agree wherever both are valid. Batch-level float reductions that feed a value (`_lu_det`, `_pf_adj_coeffs`,
  `_pf_value`) are explicit pairwise trees (`_fallback._pairwise`) so batched and vmapped programs agree bit-for-bit.
  No (B, N, N) XLA reductions: XLA's GPU compile of such a reduce fusion blows up with B and N (the det gradient at
  n = 256, B = 4096 never finished compiling while `_perm_sign` counted inversions that way). Test:
  `test_vmap_matches_batched`; benchmark: `benchmarks/vmap_bench.py` (batched vs vmapped forward / gradient).
